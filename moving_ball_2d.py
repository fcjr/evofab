...
class MovingBall :
    ...
    def move (self, dt, world):
        ...
        print 'inside movingball.move', dt, v, p
    ...
    def repair_position (self, rel_pos, other):
        ...
        if math.isinf (self.m):
            other.p = other.p.plus(rel_pos.times(-1).times(repair))
        elif math.isinf (other.m):
            self.p = self.p.plus(rel_pos.times(repair))
        else:
            self.p = self.p.plus(rel_pos.times(repair*other.m/(self.m+other.m)))
            other.p = other.p.plus(rel_pos.times(-1).times(repair*self.m/(self.m+other.m)))
    ...
    def clamp_v (self):
        ...
        if self.v.length() > self.speedlimit.length():
            self.v = self.v.normalize().times(self.speedlimit.length())
    ...
    def draw (self, window):
        print self.p, self.r, self.color
